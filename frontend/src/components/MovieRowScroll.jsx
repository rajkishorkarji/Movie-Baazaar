...
const fetchData = (fetchType, fetchParam) => {
  switch (fetchType) {
    case 'trending':    return getTrending();
    case 'popular':     return getPopular();
    case 'topRated':    return getTopRated();
    case 'genre':       return getByGenre(fetchParam);
    case 'search':      return searchMovies(fetchParam);
    case 'bollywood':   return getBollywood();
    case 'hollywood':   return getHollywood();
    case 'southIndian': return getSouthIndian();
    case 'hindiDubbed': return getHindiDubbed();
    case 'webSeries':   return getWebSeries();
    case 'recommend':   return API.get(`/recommend/${fetchParam}`);
...
const MovieRowScroll = ({ title, fetchType, fetchParam }) => {
  ...
  useEffect(() => {
    // ✅ Don't fetch if fetchParam is required but missing
    if (fetchType === 'recommend' && !fetchParam) return;
    if (fetchType === 'genre' && !fetchParam) return;

    setLoading(true);
    setError(false);

    fetchData(fetchType, fetchParam)
      .then(r => {
        const results = r.data?.results || [];
        setMovies(results);
      })
      .catch(e => {
        console.error('MovieRowScroll error:', title, e);
        setError(true);
      })
    ...
    <div className="mb-8 md:mb-10">
      ...
      <div className="relative">
        {canLeft && (
          <button
            onClick={() => scroll(-1)}
            className="hidden md:flex absolute left-2 top-1/2 -translate-y-1/2 z-10 w-10 h-10 bg-black/80 hover:bg-red-600 border border-gray-700 rounded-full items-center justify-center text-white text-xl transition-all"
          >‹</button>
        )}
        {canRight && !loading && movies.length > 0 && (
          <button
            onClick={() => scroll(1)}
            className="hidden md:flex absolute right-2 top-1/2 -translate-y-1/2 z-10 w-10 h-10 bg-black/80 hover:bg-red-600 border border-gray-700 rounded-full items-center justify-center text-white text-xl transition-all"
          >›</button>
        )}
        ...
        >
          {loading ? (
            ...
            <p className="text-gray-600 py-8 text-sm px-2">No movies found.</p>
          ) : (
