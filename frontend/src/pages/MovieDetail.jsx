...
const Avatar = ({ username, avatarUrl }) => {
  ...
  if (avatarUrl) {
    return <img src={avatarUrl} alt={username} className="w-8 h-8 rounded-full object-cover border border-white/10 flex-none" />;
  }
  return (
    <div className={`w-8 h-8 ${color} rounded-full flex items-center justify-center text-xs font-bold text-white flex-none`}>
...
const MovieDetail = () => {
  ...
  // ✅ Load movie from backend — no direct TMDB call
  useEffect(() => {
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen text-white">
      ...
      <div className="relative w-full h-[40vh] md:h-[55vh] overflow-hidden bg-[#111]">
        {backdrop && (
          <img src={backdrop} alt={movie.title} className="w-full h-full object-cover" loading="eager" onError={(e) => { e.target.style.display = 'none'; }} />
        )}
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-10 -mt-32 md:-mt-48 relative z-10">
        <div className="flex flex-col md:flex-row gap-5 md:gap-8">
          ...
          <div className="flex-none flex md:block justify-center">
            <img src={poster} alt={movie.title} className="w-36 md:w-64 rounded-xl shadow-2xl shadow-black/70 border border-white/5" onError={(e) => { e.target.src = 'https://via.placeholder.com/300x450/1a1a1a/555?text=No+Poster'; }} />
          </div>

          <div className="flex-1 pt-2 md:pt-16">
            <h1 className="text-2xl sm:text-3xl md:text-5xl font-black text-white mb-2 md:mb-3 leading-tight">{movie.title}</h1>
            ...
            <div className="mb-4 md:mb-6">
              ...
              {!user && (
                <button onClick={() => setShowAuth(true)} className="mt-2 text-xs text-red-500 hover:text-red-400 transition-colors">
                  Sign in to rate →
                </button>
              )}
            </div>
        ...
        <div className="mt-8 md:mt-10 mb-4">
          <h2 className="text-white text-base md:text-lg font-bold flex items-center gap-3 mb-4 md:mb-6">
            ...
            Reviews
            {comments.length > 0 && (
              <span className="text-xs text-gray-500 font-normal bg-white/5 px-2 py-0.5 rounded-full">{comments.length}</span>
            )}
          </h2>
