...
const STREAMING = [
  ...
  { name: 'Prime',   color: 'bg-blue-600 hover:bg-blue-500', logo: '▶', url: (t) => `https://www.primevideo.com/search/?phrase=${encodeURIComponent(t)}` },
        ...
        {[1,2,3,4,5].map(s => (
          <button key={s} type="button" disabled={readonly}
            ...
            className={`text-2xl md:text-3xl leading-none transition-all duration-100
              ...
              ${s <= active ? (hovered ? 'text-yellow-300' : 'text-yellow-400') : 'text-gray-700'}`}
          >★</button>
        ))}
      </div>
      {!readonly && (
        <span className="text-xs text-gray-500 h-4">
          {hovered ? labels[hovered] : score ? `Your rating: ${labels[score]}` : 'Tap a star to rate'}
        </span>
      )}
    </div>
  );
...
const Avatar = ({ username, avatarUrl }) => {
  ...
  if (avatarUrl) return <img src={avatarUrl} alt={username} className="w-8 h-8 rounded-full object-cover border border-white/10 flex-none" />;
  return <div className={`w-8 h-8 ${color} rounded-full flex items-center justify-center text-xs font-bold text-white flex-none`}>{username?.slice(0,2).toUpperCase()}</div>;
};
...
const MovieDetail = () => {
  ...
  useEffect(() => {
    ...
    API.get(`/movie/${id}`)
      .then(r => {
        setMovie(r.data);
        if (r.data.genres?.length) setGenreId(r.data.genres[0].id);
      })
      .catch(() => setError('Failed to load movie. Please check your connection.'))
  ...
  const handleRate = async (score) => {
    ...
    try {
      ...
      else alert('Failed to save rating. Please try again.');
    } finally {
  ...
  if (error || !movie) return (
    <div className="bg-[#0a0a0a] min-h-screen flex flex-col items-center justify-center gap-4">
      ...
      <button onClick={() => navigate(-1)} className="px-4 py-2 bg-red-600 text-white rounded-lg text-sm">← Go Back</button>
    </div>
  );
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen text-white">
      <button onClick={() => navigate(-1)}
        className="fixed top-4 left-4 z-50 flex items-center gap-2 px-3 py-2 bg-black/70 backdrop-blur-md border border-white/10 rounded-lg text-gray-400 hover:text-white text-sm transition-all">
        ← Back
      </button>

      <div className="relative w-full h-[40vh] md:h-[55vh] overflow-hidden bg-[#111]">
        {backdrop && <img src={backdrop} alt={movie.title} className="w-full h-full object-cover" loading="eager" onError={(e) => { e.target.style.display='none'; }} />}
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-10 -mt-32 md:-mt-48 relative z-10">
        <div className="flex flex-col md:flex-row gap-5 md:gap-8">
          ...
          <div className="flex-none flex md:block justify-center">
            <img src={poster} alt={movie.title} className="w-36 md:w-64 rounded-xl shadow-2xl shadow-black/70 border border-white/5"
          ...
          <div className="flex-1 pt-2 md:pt-16">
            ...
            <div className="flex items-start gap-3 mb-2 md:mb-3">
              <h1 className="flex-1 text-2xl sm:text-3xl md:text-5xl font-black text-white leading-tight">{movie.title}</h1>
              {/* ✅ Heart / Favourite button */}
              <button
                onClick={handleFavourite}
                disabled={favLoading}
                title={isFavourite ? 'Remove from favourites' : 'Add to favourites'}
                className={`flex-none mt-1 w-10 h-10 md:w-12 md:h-12 rounded-full border-2 flex items-center justify-center text-lg md:text-xl transition-all duration-200 ${
                  isFavourite
                    ? 'bg-red-600 border-red-600 text-white scale-110'
                    : 'bg-white/5 border-white/20 text-gray-400 hover:border-red-500 hover:text-red-400'
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-3 md:mb-4">
              {movie.vote_average > 0 && (
                <span className="flex items-center gap-1 bg-yellow-500/20 border border-yellow-500/30 px-2 py-0.5 rounded-full text-yellow-400 text-xs font-semibold">
                  ★ {Number(movie.vote_average).toFixed(1)}<span className="text-yellow-600 text-xs ml-0.5">TMDB</span>
                </span>
              )}
              {ratings.average && (
                <span className="flex items-center gap-1 bg-red-500/20 border border-red-500/30 px-2 py-0.5 rounded-full text-red-400 text-xs font-semibold">
                  ★ {ratings.average}<span className="text-red-600 text-xs ml-0.5">({ratings.count})</span>
                </span>
              )}
              {year    && <span className="text-gray-400 text-xs">{year}</span>}
              {runtime && <span className="text-gray-400 text-xs">{runtime}</span>}
              {movie.original_language && (
                <span className="text-gray-400 text-xs uppercase bg-white/5 px-2 py-0.5 rounded">{movie.original_language}</span>
              )}
            </div>

            <div className="flex flex-wrap gap-1.5 md:gap-2 mb-3 md:mb-5">
              {movie.genres?.map(g => (
                <span key={g.id} className="px-2 md:px-3 py-0.5 md:py-1 bg-red-600/20 border border-red-600/30 rounded-full text-red-400 text-xs font-medium">{g.name}</span>
              ))}
            </div>

            <p className="text-gray-300 text-sm leading-relaxed mb-4 md:mb-6 max-w-2xl">{movie.overview}</p>

            <div className="mb-4 md:mb-6">
              <p className="text-gray-500 text-xs uppercase tracking-widest mb-2">
                Your Rating {ratingLoading && <span className="ml-2 text-gray-600 normal-case">Saving...</span>}
              </p>
              <StarRating score={ratings.user_score} onRate={handleRate} />
              {!user && <button onClick={() => setShowAuth(true)} className="mt-2 text-xs text-red-500 hover:text-red-400 transition-colors">Sign in to rate →</button>}
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {STREAMING.map(s => (
                  <a key={s.name} href={s.url(movie.title)} target="_blank" rel="noopener noreferrer"
                    className={`flex items-center gap-1.5 px-3 md:px-5 py-2 md:py-2.5 rounded-lg text-white text-xs md:text-sm font-semibold transition-all ${s.color} shadow-lg`}>
            ...
            {trailer && (
              <a href={`https://www.youtube.com/watch?v=${trailer.key}`} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 border border-white/10 rounded-lg text-white text-sm font-medium transition-all">
                ▶ Watch Trailer
              </a>
            )}
          </div>
          ...
          {ratings.count > 0 ? (
            <div className="flex items-center gap-4 md:gap-6">
              <div className="text-center">
                ...
                <p className="text-gray-600 text-xs mt-1">{ratings.count} rating{ratings.count !== 1 ? 's' : ''}</p>
              </div>
        ...
        <div className="mt-8 md:mt-10 mb-4">
          <h2 className="text-white text-base md:text-lg font-bold flex items-center gap-3 mb-4 md:mb-6">
            ...
            Reviews
            {comments.length > 0 && <span className="text-xs text-gray-500 font-normal bg-white/5 px-2 py-0.5 rounded-full">{comments.length}</span>}
          </h2>
          ...
          {user ? (
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 space-y-2 md:space-y-3">
                  <textarea value={commentText} onChange={e => setCommentText(e.target.value)}
                    ...
                    className="w-full bg-white/5 border border-white/10 focus:border-red-500/50 rounded-xl px-3 md:px-4 py-3 text-white text-sm outline-none transition-colors placeholder-gray-600 resize-none" />
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-700">{commentText.length}/1000</span>
                    <button type="submit" disabled={submitting || !commentText.trim()}
                      className="px-4 md:px-5 py-2 bg-red-600 hover:bg-red-500 disabled:opacity-40 text-white text-sm font-bold rounded-lg transition-all">
            ...
            <button onClick={() => setShowAuth(true)}
              className="w-full mb-6 md:mb-8 py-4 border border-dashed border-white/15 rounded-xl text-gray-500 hover:text-gray-300 hover:border-white/30 text-sm transition-all">
          ...
          {comments.length === 0 ? (
            ...
            <div className="space-y-3 md:space-y-4">
              {comments.map(c => (
                <div key={c.id} className="group flex gap-3 bg-white/[0.03] hover:bg-white/[0.05] border border-white/5 hover:border-white/10 rounded-xl px-4 md:px-5 py-3 md:py-4 transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      ...
                      {c.is_mine && <span className="text-xs text-red-500/70 bg-red-500/10 px-1.5 py-0.5 rounded">you</span>}
                    </div>
                    <p className="text-gray-300 text-sm leading-relaxed break-words">{c.body}</p>
                  </div>
                  {c.is_mine && (
                    <button onClick={() => handleDeleteComment(c.id)}
                      className="opacity-0 group-hover:opacity-100 text-gray-700 hover:text-red-500 text-xs transition-all flex-none self-start mt-0.5 p-1">✕</button>
                  )}
                </div>
              ))}
            </div>
